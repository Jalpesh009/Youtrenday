...
function youtrenday_scripts() {
	wp_enqueue_style( 'youtrenday-style', get_stylesheet_uri(), array(), '1.0.1' );
...
function custom_youtrenday_enqueue(){ 
	...
	wp_enqueue_style( 'customnew', get_template_directory_uri() . '/assets/css/custom.css', array(), '1.0.0', 'all'  );
	wp_enqueue_style( 'landing-page', get_template_directory_uri() . '/assets/css/landing-page.css', '', null );
	// wp_enqueue_style( 'youtrendaystyle-style', get_stylesheet_uri(), array(), '1.0.1', 'all' );
	wp_enqueue_style('responsive-css', get_template_directory_uri() . '/assets/css/responsive.css', array(), '1.0.0', 'all' );
	wp_enqueue_style('bootstrap-css', get_template_directory_uri() . '/assets/css/bootstrap.css', array(), '1.0.0', 'all' ); 
	wp_enqueue_style('fancybox-css', "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css", array(), '1.0.0', 'all' );
	wp_enqueue_style('slick-css', get_template_directory_uri() . '/assets/css/slick.css', '', null );  
	wp_enqueue_style('select2-min-css', get_template_directory_uri() . '/assets/css/select2.min.css', array(), '1.0.0', 'all' ); 
	wp_enqueue_style('croppie-css', get_template_directory_uri() . '/assets/css/croppie.css', '', null );  

	// js
	wp_enqueue_script('bootstrap-js', get_template_directory_uri() . '/assets/js/bootstrap.min.js', array(), '1.0.0', 'all' ); 
	wp_enqueue_script('custom-js', get_template_directory_uri() . '/assets/js/custom.js', array(), '1.0.0', 'all' );
	wp_enqueue_script('slick-min-js', get_template_directory_uri() . '/assets/js/slick.min.js', array(), '1.0.0', 'all' );  
	wp_enqueue_script('select2-min-js', get_template_directory_uri() . '/assets/js/select2.min.js', array(), '1.0.0', 'all' );   
	wp_enqueue_script('platform-js', get_template_directory_uri() . '/assets/js/platform.js', array(), '1.0.0', 'all');   
	// wp_enqueue_script('jQuery-inViewmin-js', get_template_directory_uri() . '/assets/js/jQuery-inView.min.js', '', null );   

	wp_enqueue_script('croppie-js', get_template_directory_uri() . '/assets/js/croppie.js', '', null );
...
function my_nav_menu_profile_link( $items, $args) { 	 		
	if($args->theme_location == 'header_profile_menu'){ 
		...
		// 	$name = substr($text, 0, 7);
		// }  
		$user_link = '<li class="rigtsidebar_username"><span>Welcome,</span><span class="user_email_floating1 "></br> '. $first_name . '</span></li> <li><a href="'.home_url( '/members/' . bp_core_get_username( get_current_user_id() ) . '/profile/edit/group/1' ) .'">Profile</a></li>';
...
function get_posts_shortcode($posttype){  
	...
	<div class="container user-profile-musics mt-5">
		<h3 class="text-center"><?php echo $user_info->display_name; ?>'s <?php echo $posttype; ?>s </h3> 
		...
		while ($loop -> have_posts()) : $loop -> the_post(); 
			...
			if( $musicMedia_from == 'media_system' ){
				...
				$media = '<div class="embed-responsive">   
					<iframe src="'. $musicMedia_url .'" width="300" height="380" frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>
				</div>';   
			} 
	...
	$html .= '<div class="home-music-list">
		<div class="home_item pId_'. $post_id .'">'; 
			...
			if( $musicMedia_from == 'media_system' || $musicMedia_from === 'comp_from_computer' ){
				...
				$html .= '<iframe class="'.$video_cls.'" src="'.$musicMedia_url .'" width="100%"  frameborder="0" allowtransparency="true" allow="encrypted-media"></iframe>';   
		    } 			
			$html .= '</div>
	</div>'; 	 
	$output = str_replace(array("\r\n", "\r"), "\n", $html);
...
function show_home_postsdata($post_id){
	...
	if($post_likes_count != null && $post_likes_count){
	...
	if($post_dislikes_count != null && $post_dislikes_count){
	...
	$post_status= wp_get_post_terms($post_id, 'competitionvideo_status', array( 'fields' => 'names' ) ); 
	if($posttype  == 'competition_video'){  
		if($post_status[0]  == 'Voting' ){
			$html .= get_home_mediaall_html($post_id);
			/* $html .=  '<ul class="p-0 text-center like_share_dislike competition_voting" data-pid="'.$post_id .'">
				<li class="like_mus_comp com_cls"> 
					<span class="likes_count">'.$like_count .'</span>
					<a href="javascript:void(0);" data-totallike="'.$post_likes_count.'" data-postaction="'.$action.'" class="d-inline '.$like_cls .'" data-postid="'. $post_id .'" data-userid="'.get_current_user_id() .'">Trend-it</a>
				</li>
				<li class="dislike_mus_comp com_cls">
					<span class="dislikes_count">'. $dislike_count .'</span>
					<a href="javascript:void(0);" data-totallike="'.$post_dislikes_count.'" data-postaction="'.$dislike_action .'" class="d-inline '.$dislike_cls.'" data-postid="'.$post_id .'" data-userid="'. get_current_user_id().'"> End-it</a>
				</li>
				<li class="share_mus_comp com_cls"> 
					<a href="javascript:void(0);" class="share-post" data-postid="'. $post_id .'" data-userid="'. get_current_user_id() .'" >Share-it</a>
				</li>
			</ul>';  */

			$html .=  '<ul class="p-0 text-center like_share_dislike" data-pid="'. get_the_ID() .'">
...
function custom_login_member() {
	...
	if(isset($_POST['email']) && wp_verify_nonce($_POST['pippin_login_nonce'], 'pippin-login-nonce')) {
		...
		if(!$user) {
			...
			custom_wp_errors()->add('empty_email', __('Invalid email'));
		}
		...
		if(!wp_check_password($_POST['password'], $user->user_pass, $user->ID)) {
			...
			custom_wp_errors()->add('empty_password', __('Incorrect password'));
		}
...
function validate_youtubeURL_music_1( $result, $value, $form, $field ) {
	$pattern = "/^(https?\:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/watch\?v\=\w+$/"; 
...
function showdata_in_modal_ajax(){    
	...
	if( $musicMedia_from == 'media_system' || $musicMedia_from == 'comp_from_computer' ){
		...
		preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $musicMedia_url, $match);
		$youtube_id = $match[1]; 
		$thumb .= 'https://img.youtube.com/vi/'. $youtube_id .'/mqdefault.jpg';      
	}elseif( $musicMedia_from == 'soundcloud' ||  
		$musicMedia_from == 'spotify' || 
		$musicMedia_from == 'comp_from_soundcloud' || 
		$musicMedia_from == 'comp_spotify' ){ 
		$thumb .=   $musicMedia_url  ;
